...
#-------------------------------------------------------------------------------
#set your parameters
power= 8  # we raise the adjacency matrix to this power to get rid of weak edges, hence emphasizing only strong correlations and ensure that the network degree follow a scale free distribution, recommended to be less than 15 for unsigned or signed hybrid networks, and less than 30 for signed networks  
minModuleSize = 20 #minimum module size is the minimum size/ number of genes per module 
...
mapping_file= read.delim("input/idmapping.csv", header = T, sep= ",") #needs to be 2 columns that are the query id and the matched id of interest
